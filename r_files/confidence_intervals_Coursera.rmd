...
```{r set-seed}
set.seed(1430988)                 # make sure to change the seed
```
...
samp %>%
  summarise(pop_mu = mean(area), pop_med = median(area), 
            sigma = sd(area), pop_iqr = IQR(area),
            pop_min = min(area), pop_max = max(area),
            pop_q1 = quantile(area, 0.25),  # first quartile, 25th percentile  
            pop_q3 = quantile(area, 0.75))  # third quartile, 75th percentile)

#Distribution looks normal. Typical size looks to be about 1500, as it is relatively normal 
```
...
>Answer is The sample distribution must be nearly normal
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
ci_99_data <- data.frame(ci_99_id = c(1:50, 1:50),
                      ci_99_bounds = c(ci_99$lower, ci_99$upper),
                      capture_mu = c(ci_99$capture_mu, ci_99$capture_mu))

ggplot(data = ci_99_data, aes(x = ci_99_bounds, y = ci_99_id, 
                           group = ci_99_id, color = capture_mu)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line
```
