...
nc1 <- nc %>%
  na.omit() %>%
  group_by(habit, weight)
ggplot(data=nc1, aes(y=weight, x=habit)) +geom_boxplot()
...
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ci", method = "theoretical",conf_level=.95)
```
...
inference(y=weeks, data=nc, statistic="mean", type="ci", conf_level=.99, method="theoretical")
```
...
inference(y=weeks, data=nc, statistic="mean", type="ci", conf_level=.90, method="theoretical")
