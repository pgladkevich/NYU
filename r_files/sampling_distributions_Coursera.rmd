...
1.  Which of the following is **false**? Answer: 50\% of houses in Ames are smaller than 1,499.69 square feet.
...
samp1 %>%
  summarise(xbar = mean(area), s_median = median(area),
            s = sd(area), samp_iqr = IQR(area), samp_min = min(area), samp_max = max(area),
...
#There are 15,000 sample's that are taken with replacement from the 2930 values that represent the population. The mean is centered on 1500 square ft. of home.
...
sample_means_small <- ames %>%
  rep_sample_n(size = 10, replace = TRUE, reps = 25) %>%
    summarise(x_bar=mean(area))
#There are 25 observations and each one is a mean of a sample size ten taken from the general population. 
...
Answer is Each element represents a mean square footage from a simple random sample of 10 houses.
...
#I would guess it to be 180,000 and it is unimodal, symmetric but with a slight right skew. 180796 is the real mean.
```
...
Answer is The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`)
